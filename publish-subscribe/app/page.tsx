...
const workflowSource = readFileSync(
  join(process.cwd(), "workflows/publish-subscribe.ts"),
  "utf-8"
);
...
const workflowCode = extractFunctionBlock(workflowSource, "export async function publishSubscribeFlow(");
...
function buildStepLineMap(code: string): StepLineMap {
  ...
  return {
    registerSubscribers: collectFunctionBlock(lines, "async function registerSubscribers("),
    filterSubscribers: collectFunctionBlock(lines, "async function filterSubscribers("),
    deliverToSubscribers: collectFunctionBlock(lines, "async function deliverToSubscribers("),
    summarizeDelivery: collectFunctionBlock(lines, "async function summarizeDelivery("),
  };
}
      ...
      <main id="main-content" className="mx-auto max-w-4xl" role="main">
        <header className="mb-12">
          ...
          <p className="max-w-2xl text-lg text-gray-900">
            Publish a message to a topic and deliver it only to subscribers that
            registered interest in that topic. Unlike fan-out (which broadcasts
            to all targets), pub/sub uses{" "}
            <code className="rounded border border-gray-300 bg-background-200 px-2 py-0.5 font-mono text-sm">
              topic-based filtering
            </code>{" "}
            so each subscriber only receives messages matching its subscriptions.
          </p>
        </header>

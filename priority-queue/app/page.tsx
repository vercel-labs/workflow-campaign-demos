...
const workflowSource = readFileSync(
  join(process.cwd(), "workflows/priority-queue.ts"),
  "utf-8"
);
...
const workflowCode = extractFunctionBlock(workflowSource, "export async function priorityQueueFlow(");

const stepCode = [
  extractFunctionBlock(workflowSource, "async function sortTasks("),
  "",
  extractFunctionBlock(workflowSource, "async function processTask("),
].join("\n");
...
function buildWorkflowLineMap(code: string): WorkflowLineMap {
  ...
  return {
    receive: collectUntil(
      lines,
      "await emitEvent({ type: \"tasks_received\"",
      (line) => line.includes("tasks_received")
    ),
    sort: collectUntil(
      lines,
      "const sorted = await sortTasks(",
      (line) => line.includes("sortTasks(")
    ),
    process: collectUntil(
      lines,
...
function buildStepLineMap(code: string): StepLineMap {
  ...
  return {
    sortTasks: collectFunctionBlock(lines, "async function sortTasks("),
    processTask: collectFunctionBlock(lines, "async function processTask("),
  };
}
      ...
      <main id="main-content" className="mx-auto max-w-4xl" role="main">
        <header className="mb-12">
          ...
          <p className="max-w-2xl text-lg text-gray-900">
            Sort incoming tasks by priority level and process them in order —
            urgent first, then high, medium, and low. The workflow durably
            sorts the queue, processes each task as a separate step, and

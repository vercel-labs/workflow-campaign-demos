...
export function DetourDemo() {
  ...
  const [deployId, setDeployId] = useState("deploy-042");
  ...
  const currentStep = useMemo(() => {
    const running = [...events].reverse().find((event) => event.type === "step_running");
    return typeof running?.step === "string" ? running.step : "waiting";
  }, [events]);
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/detour", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ deployId, qaMode }),
  ...
  }, [connectToStream, deployId, qaMode]);

  return (
    <section className="space-y-4 rounded-lg border border-gray-300 bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-gray-950">Detour</h2>
          <p className="text-sm text-gray-600">
            Route a deployment through the QA detour when extra review is enabled.
          </p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-[2fr,1fr]">
        <label className="space-y-1 text-sm text-gray-700">
          <span className="font-medium">Deploy ID</span>
          <input
      ...
      <ol className="space-y-2 rounded-md border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700">
        {events.length === 0 ? <li>No pipeline activity yet.</li> : null}

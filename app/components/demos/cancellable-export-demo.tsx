...
export function CancellableExportDemo({
  sectionNames,
  sectionContent,
}: {
  sectionNames: string[];
  ...
  const statusText = useMemo(() => {
    switch (state) {
      case "idle":
        return "0 sections ready";
      case "running":
        return `Generating section ${generatedCount + 1} of ${total}...`;
      case "cancelling":
        return `Cancelling after ${generatedCount} of ${total} sections`;
      case "completed":
        return `All ${total} sections generated`;
      case "cancelled":
        return `Cancelled — ${cancelCutoff ?? generatedCount} of ${total} sections generated`;
    }
  }, [state, generatedCount, total, cancelCutoff]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="relative min-h-[400px]">
          <div className="absolute inset-0 rounded-lg border border-gray-400 bg-background-100 overflow-hidden flex flex-col">
            ...
            <div
              ref={docScrollRef}
              className="flex-1 overflow-y-scroll p-4 min-h-0 [color-scheme:dark]"
            >
              {generatedCount === 0 ? (

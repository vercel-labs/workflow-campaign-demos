    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-gray-400/70 bg-background-100 p-3">
        <div className="flex flex-wrap items-center gap-2">
          <label className="sr-only" htmlFor="approval-amount">
            Expense amount
          </label>
          <select
            id="approval-amount"
      ...
      {isDone ? (
        <div className={`rounded-lg border p-3 text-xs ${
          ...
          {demoStatus === "approved"
            ? "Expense approved — workflow completed successfully."
            : demoStatus === "rejected"
              ? "Expense rejected — workflow stopped at the rejecting level."
              : "All levels expired — no approval received within timeout windows."}
        </div>
      ) : null}
    </div>
  );

...
export function MessageHistoryDemo() {
  ...
  const [subject, setSubject] = useState("Urgent: checkout outage");
  const [body, setBody] = useState("Customers report checkout is down and payments are failing.");
  ...
  const finalEnvelope = useMemo(() => {
    const done = [...events].reverse().find((event) => event.type === "done");
    return done?.envelope ? JSON.stringify(done.envelope, null, 2) : "No envelope yet";
  }, [events]);
  ...
  }, [body, connectToStream, failAtStep, subject]);

  return (
    <section className="space-y-4 rounded-lg border border-gray-300 bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-gray-950">Message History</h2>
          <p className="text-sm text-gray-600">
            Route a support ticket and watch the envelope history build step by step.
          </p>
        </div>
      ...
      <label className="space-y-1 text-sm text-gray-700">
        <span className="font-medium">Fail at Step</span>
        <select
          className="w-full rounded-md border border-gray-300 px-3 py-2"
        ...
        >
          ...
          <option value="normalizeTicket">normalizeTicket</option>
          <option value="classifySeverity">classifySeverity</option>
          <option value="chooseRoute">chooseRoute</option>
          <option value="dispatchTicket">dispatchTicket</option>
        </select>
      </label>
      ...
      <div className="rounded-md border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700">
        <p className="font-medium text-gray-900">Final Envelope</p>
        <pre className="mt-2 overflow-auto whitespace-pre-wrap text-xs">{finalEnvelope}</pre>
      </div>

      {error ? <p className="text-sm text-rose-700">{error}</p> : null}

      <ol className="space-y-2 rounded-md border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700">
        {events.length === 0 ? <li>No routing history yet.</li> : null}

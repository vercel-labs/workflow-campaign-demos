...
export function MessageTranslatorDemo() {
  ...
  const [messageId, setMessageId] = useState("MSG-001");
  const [sourceFormat, setSourceFormat] = useState<SourceFormat>("xml");
  ...
  const mappedFieldCount = useMemo(
    () => events.filter((event) => event.type === "field_mapped").length,
    [events],
  );
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/message-translator", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ messageId, sourceFormat }),
  ...
  }, [connectToStream, messageId, sourceFormat]);

  return (
    <section className="space-y-4 rounded-lg border border-gray-300 bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-gray-950">Message Translator</h2>
          <p className="text-sm text-gray-600">
            Translate a legacy payload into canonical fields and deliver it downstream.
          </p>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <p className="text-sm text-gray-600">Mapped fields: {mappedFieldCount}</p>
      </div>
      ...
      <ol className="space-y-2 rounded-md border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700">
        {events.length === 0 ? <li>No translator events yet.</li> : null}

...
function parseSseChunk(rawChunk: string): BulkheadEvent | null {
  ...
  try {
    return JSON.parse(payload) as BulkheadEvent;
  } catch {
    return null;
  }
}
...
export function BulkheadDemo() {
  const [jobId, setJobId] = useState("job-bulkhead");
  const [itemsText, setItemsText] = useState("order-1, order-2, order-3, order-4, order-5, order-6");
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/bulkhead", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ jobId, items, maxConcurrency }),
      });
  ...
  }, [connectToStream, itemsText, jobId, maxConcurrency]);

  const summary = useMemo(() => {
    const finalEvent = [...events].reverse().find((event) => event.type === "done");
    return finalEvent?.summary ?? null;
  }, [events]);
  ...
  return (
    <section className="rounded-xl border border-gray-300 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-950">Bulkhead</h2>
          <p className="text-sm text-gray-600">
            Splits work into isolated compartments so one failed item does not take down the whole batch.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-[1fr_1fr_160px_140px]">
        ...
        <label className="text-sm">
          <span className="mb-1 block font-medium text-gray-700">Parallelism</span>
          <input
            className="w-full rounded-md border border-gray-300 px-3 py-2"
            type="number"
            min={1}
            ...
            onChange={(event) => setMaxConcurrency(Number(event.target.value) || 3)}
            disabled={isRunning}
          />
        </label>
        <button
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
          <p>Successes</p>
          <p className="mt-1 text-lg font-semibold text-gray-900">{summary?.succeeded ?? 0}</p>
        </div>
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
          <p>Failures</p>
          <p className="mt-1 text-lg font-semibold text-gray-900">{summary?.failed ?? 0}</p>
        </div>
      </div>

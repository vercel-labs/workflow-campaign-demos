...
import { AggregatorDemo } from "@/app/components/demos/aggregator-demo";
...
const SLUG = "aggregator";

export const aggregatorAdapter: DemoAdapter = {
  slug: SLUG,
  title: "Aggregator",
  ...
  async renderDemo() {
    ...
    return createElement(AggregatorDemo);
  },

  async getCodeBundle(): Promise<DemoCodeFile[]> {
    const files: DemoCodeFile[] = [
      {
        path: `${SLUG}/workflows/aggregator.ts`,
        role: "workflow",
        contents: readDemoFile(SLUG, "workflows/aggregator.ts"),
      },
      {
      ...
      {
        path: `${SLUG}/app/api/aggregator/route.ts`,
        role: "api",
        contents: readDemoFile(SLUG, "app/api/aggregator/route.ts"),
      },
      {
  ...
  apiRoutes: [
    {
      ...
      kind: "start",
      load: () => import("@/app/api/aggregator/route"),
    },
    {

...
import { HedgeRequestDemo } from "@/app/components/demos/hedge-request-demo";
...
const SLUG = "hedge-request";

export const hedgeRequestAdapter: DemoAdapter = {
  slug: SLUG,
  title: "Hedge Request",
  ...
  async renderDemo() {
    ...
    return createElement(HedgeRequestDemo);
  },

  async getCodeBundle(): Promise<DemoCodeFile[]> {
    const files: DemoCodeFile[] = [
      {
        path: `${SLUG}/workflows/hedge-request.ts`,
        role: "workflow",
        contents: readDemoFile(SLUG, "workflows/hedge-request.ts"),
      },
      {
      ...
      {
        path: `${SLUG}/app/api/hedge-request/route.ts`,
        role: "api",
        contents: readDemoFile(SLUG, "app/api/hedge-request/route.ts"),
      },
      {
  ...
  apiRoutes: [
    {
      ...
      kind: "start",
      load: () => import("@/app/api/hedge-request/route"),
    },
    {

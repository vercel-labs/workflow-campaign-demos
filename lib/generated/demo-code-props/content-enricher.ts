...
export function getContentEnricherCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "content-enricher/workflows/content-enricher.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      "baseLookup": findBestGeneratedRange(workflowCode, "baseLookup", workflowFallbackLines),
      "fanOut": findBestGeneratedRange(workflowCode, "fanOut", workflowFallbackLines),
      "merge": findBestGeneratedRange(workflowCode, "merge", workflowFallbackLines),
    },
    stepCode: secondaryCode,
    stepHtmlLines: secondaryHtmlLines,
    stepLineMap: {
      "fetchSteps": findBestGeneratedRange(secondaryCode, "fetchSteps", secondaryFallbackLines),
      "merge": findBestGeneratedRange(secondaryCode, "merge", secondaryFallbackLines),
    },
  };
}

...
export function getTransactionalOutboxCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "transactional-outbox/workflows/transactional-outbox.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      "markSentCall": findBestGeneratedRange(workflowCode, "markSentCall", workflowFallbackLines),
      "persistCall": findBestGeneratedRange(workflowCode, "persistCall", workflowFallbackLines),
      "publishCall": findBestGeneratedRange(workflowCode, "publishCall", workflowFallbackLines),
      "relayCall": findBestGeneratedRange(workflowCode, "relayCall", workflowFallbackLines),
    },
    stepLineMap: {
      "markSentCall": findBestGeneratedRange(secondaryCode, "markSentCall", secondaryFallbackLines),
      "persistCall": findBestGeneratedRange(secondaryCode, "persistCall", secondaryFallbackLines),
      "publishCall": findBestGeneratedRange(secondaryCode, "publishCall", secondaryFallbackLines),
      "relayCall": findBestGeneratedRange(secondaryCode, "relayCall", secondaryFallbackLines),
    },
  };
}

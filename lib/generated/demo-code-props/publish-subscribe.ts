...
export function getPublishSubscribeCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "publish-subscribe/workflows/publish-subscribe.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      "deliver": findBestGeneratedRange(workflowCode, "deliver", workflowFallbackLines),
      "filter": findBestGeneratedRange(workflowCode, "filter", workflowFallbackLines),
      "register": findBestGeneratedRange(workflowCode, "register", workflowFallbackLines),
      "summarize": findBestGeneratedRange(workflowCode, "summarize", workflowFallbackLines),
    },
    stepLineMap: {
      "deliverToSubscribers": findBestGeneratedRange(secondaryCode, "deliverToSubscribers", secondaryFallbackLines),
      "filterSubscribers": findBestGeneratedRange(secondaryCode, "filterSubscribers", secondaryFallbackLines),
      "registerSubscribers": findBestGeneratedRange(secondaryCode, "registerSubscribers", secondaryFallbackLines),
      "summarizeDelivery": findBestGeneratedRange(secondaryCode, "summarizeDelivery", secondaryFallbackLines),
    },
  };
}

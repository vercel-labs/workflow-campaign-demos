...
export function getGuaranteedDeliveryCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "guaranteed-delivery/workflows/guaranteed-delivery.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      "forLoop": findBestGeneratedRange(workflowCode, "forLoop", workflowFallbackLines),
    },
    stepLineMap: {
      "deliverMessage": findBestGeneratedRange(secondaryCode, "deliverMessage", secondaryFallbackLines),
      "finalizeDelivery": findBestGeneratedRange(secondaryCode, "finalizeDelivery", secondaryFallbackLines),
    },
  };
}

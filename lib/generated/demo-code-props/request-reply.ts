...
export function getRequestReplyCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "request-reply/workflows/request-reply.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      "emitDone": findBestGeneratedRange(workflowCode, "emitDone", workflowFallbackLines),
      "sendLoop": findBestGeneratedRange(workflowCode, "sendLoop", workflowFallbackLines),
    },
    stepLineMap: {

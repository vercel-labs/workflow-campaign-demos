...
export function getEventGatewayCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "event-gateway/workflows/event-gateway.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      ...
      "returnShipped": findBestGeneratedRange(workflowCode, "returnShipped", workflowFallbackLines),
      "returnTimeout": findBestGeneratedRange(workflowCode, "returnTimeout", workflowFallbackLines),
      "ship": findBestGeneratedRange(workflowCode, "ship", workflowFallbackLines),
    },
    stepCode: secondaryCode,
    stepHtmlLines: secondaryHtmlLines,
    stepLineMap: {
      "shipFetch": findBestGeneratedRange(secondaryCode, "shipFetch", secondaryFallbackLines),
    },
  };
}

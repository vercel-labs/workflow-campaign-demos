...
export function getBatchProcessorCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "batch-processor/workflows/batch-processor.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      "checkpoint": findBestGeneratedRange(workflowCode, "checkpoint", workflowFallbackLines),
      "checkpointComment": findBestGeneratedRange(workflowCode, "checkpointComment", workflowFallbackLines),
      "returnDone": findBestGeneratedRange(workflowCode, "returnDone", workflowFallbackLines),
    },
    stepCode: secondaryCode,
    stepHtmlLines: secondaryHtmlLines,
    stepLineMap: {
      "processItem": findBestGeneratedRange(secondaryCode, "processItem", secondaryFallbackLines),
      "promiseAll": findBestGeneratedRange(secondaryCode, "promiseAll", secondaryFallbackLines),
    },
  };
}

...
export function getHedgeRequestCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "hedge-request/workflows/hedge-request.ts"), "utf-8");
  ...
  return {
    ...
    stepLineMap: {
      "callProvider": findBestGeneratedRange(secondaryCode, "callProvider", secondaryFallbackLines),
    },
  };
}

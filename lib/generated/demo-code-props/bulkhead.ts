...
export function getBulkheadCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "bulkhead/workflows/bulkhead.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      "allSettled": findBestGeneratedRange(workflowCode, "allSettled", workflowFallbackLines),
      "pacing": findBestGeneratedRange(workflowCode, "pacing", workflowFallbackLines),
      "summarize": findBestGeneratedRange(workflowCode, "summarize", workflowFallbackLines),
    },
    stepLineMap: {
      "failure": findBestGeneratedRange(secondaryCode, "failure", secondaryFallbackLines),
      "processing": findBestGeneratedRange(secondaryCode, "processing", secondaryFallbackLines),
      "success": findBestGeneratedRange(secondaryCode, "success", secondaryFallbackLines),
    },
  };
}

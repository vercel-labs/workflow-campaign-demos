...
export function getNormalizerCodeProps(): Record<string, unknown> {
  ...
  return {
    ...
    workflowLineMap: {
      "detectCall": findBestGeneratedRange(workflowCode, "detectCall", workflowFallbackLines),
      "emitCall": findBestGeneratedRange(workflowCode, "emitCall", workflowFallbackLines),
      "parseCall": findBestGeneratedRange(workflowCode, "parseCall", workflowFallbackLines),
    },
    stepLineMap: {
      "detect": findBestGeneratedRange(secondaryCode, "detect", secondaryFallbackLines),
      "emit": findBestGeneratedRange(secondaryCode, "emit", secondaryFallbackLines),
      "parse": findBestGeneratedRange(secondaryCode, "parse", secondaryFallbackLines),
    },
  };
}

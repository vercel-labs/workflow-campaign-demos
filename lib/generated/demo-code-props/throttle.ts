...
export function getThrottleCodeProps(): Record<string, unknown> {
  const source = readFileSync(join(process.cwd(), "throttle/workflows/throttle.ts"), "utf-8");
  ...
  return {
    ...
    workflowLineMap: {
      ...
      "evaluate": findBestGeneratedRange(workflowCode, "evaluate", workflowFallbackLines),
    },
    stepLineMap: {
      "evaluateRequest": findBestGeneratedRange(secondaryCode, "evaluateRequest", secondaryFallbackLines),
    },
  };
}

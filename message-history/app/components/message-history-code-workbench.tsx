...
export type MessageHistoryCodeWorkbenchProps = {
  leftPane: PaneState;
  rightPane: PaneState;
};

export function MessageHistoryCodeWorkbench({
  leftPane,
  rightPane,
}: MessageHistoryCodeWorkbenchProps) {
  return (
      ...
      <div className="flex items-center justify-between border-b border-gray-300 bg-background-100 px-4 py-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={handleCopy}
            className="cursor-pointer rounded-md border border-gray-400 px-2.5 py-1 text-xs font-medium text-gray-900 transition-colors hover:border-gray-300 hover:text-gray-1000 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-700/60"
          >
            {copyState === "copied"
              ? "Copied"
              : copyState === "failed"
